<h3>Cập nhật đơn hàng: <?php echo $order['code'] ?></h3>
			...
			<table class="table table-bordered">
				...
				<?php if ($_SERVER['REQUEST_METHOD'] == 'POST'): ?>
					<?php if (isset($_POST['product_id']) && isset($_POST['quantity'])): ?>
						<?php
						$count = count(set_value('product_id'));
						for ($i = 0; $i < $count; $i++):
							?>
							<tr>
								<td>
						...
						for ($i = 0; $i < $count; $i++):
							...
							<tr>
								...
								<td>
									<input type="number" class="form-control" name="quantity[]"
										   id="<?php echo 'quantity' . $i ?>"
										   value="<?php echo $order_product[$i]['quantity'] ?>"
									>
								</td>
								<td></td>
								<td id="<?php echo 'total_each' . $i ?>"></td>
						...
						<?php foreach ($customer as $data): ?>
							<option value="<?php echo $data['id'] ?>"
									...
									if ($_SERVER['REQUEST_METHOD'] == 'POST') {
										echo set_value('customer_id') == $data['id'] ? 'selected' : '';
									} else {
										if ($order['customer_id']) {
											echo $order['customer_id'] == $data['id'] ? 'selected' : '';
										}
									}
					...
					<?php if (count($repository) > 0): ?>
						<?php foreach ($repository as $data): ?>
							<option value="<?php echo $data['id'] ?>"
									<?php
									if ($_SERVER['REQUEST_METHOD'] == 'POST') {
										echo set_value('repository_id') == $data['id'] ? 'selected' : '';
									} else {
										if ($order['repository_id']) {
											echo $order['repository_id'] == $data['id'] ? 'selected' : '';
										}
									}
									?>
					...
					<div class="form-check col-lg-6">
						<input class="form-check-input" type="radio" name="payment_method" id="payment_method1"
							   value="1"
								<?php
								if ($_SERVER['REQUEST_METHOD'] == 'POST') {
									echo set_value('payment_method') == 1 ? 'checked' : '';
								} else {
									if ($order['payment_method']) {
										echo $order['payment_method'] == 1 ? 'checked' : '';
									}
								}
								?>
						>
					...
					<div class="form-check col-lg-6">
						<input class="form-check-input" type="radio" name="payment_method" id="payment_method2"
							   value="2"
								<?php
								if ($_SERVER['REQUEST_METHOD'] == 'POST') {
									echo set_value('payment_method') == 2 ? 'checked' : '';
								} else {
									if ($order['payment_method']) {
										echo $order['payment_method'] == 2 ? 'checked' : '';
									}
								}
								?>
						>
					...
					<div class="form-check col-lg-6">
						<input class="form-check-input" type="radio" name="payment_status" id="payment_status1"
							   value="1"
								<?php
								if ($_SERVER['REQUEST_METHOD'] == 'POST') {
									echo set_value('payment_status') == 1 ? 'checked' : '';
								} else {
									if ($order['payment_method']) {
										echo $order['payment_status'] == 1 ? 'checked' : '';
									}
								}
								?>
						>
