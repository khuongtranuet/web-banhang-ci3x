...
class Product_model extends CI_Model
{
	...
	public function product_review($id)
	{
		$this->db->select('
			...
			p.content
		');
		$this->db->from(TBL_PRODUCT_REVIEWS . ' AS p');
		$this->db->join(TBL_CUSTOMERS . ' AS c', 'c.id =p.customer_id');
		$this->db->where('p.product_id = ' . $id);
		$query = $this->db->get();
		return $query->result_array();
	}

	public function strange_customer($fullname, $mobile, $email)
	{
		$data = array(
			'fullname' => $fullname,
			'mobile' => $mobile,
	...
	public function insert_review($review)
	{
		...
		$this->db->insert(TBL_PRODUCT_REVIEWS, $data);
		return $this->db->insert_id();
	}

	public function insert_image_review($product_review_id, $path)
	{
		$data = array(
			'product_review_id' => $product_review_id,
			'path' => $path
		);
		$this->db->insert(TBL_PRODUCT_REVIEW_IMAGES, $data);
	}
}
